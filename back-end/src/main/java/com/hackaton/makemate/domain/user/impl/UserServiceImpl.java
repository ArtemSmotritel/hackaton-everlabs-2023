...
public class UserServiceImpl implements UserService {
    ...
    private List<User> matchAlgorithm(User sender, List<User> targets) {
        ...
        List<User> filteredTargets = targets.stream()
                .filter(user -> !Objects.equals(sender.getId(), user.getId()) || !sender.getMatches().contains(targets))
                .toList();
    ...
    private double calculateMatchScore(User sender, User target) {
        ...
        List<Interest> commonInterests = sender.getInterests().stream()
                .filter(target.getInterests()::contains)
                .toList();
